...
describe("UIコンポーネント (UI Components)", () => {
  describe("HistoryItem（検索履歴アイテム）", () => {
    ...
    describe("正常系テスト", () => {
      it("ルート情報・メタ情報・各種アイコンが重なりなく正常に描画される", () => {
      ...
      it("削除ボタン押下時に選択イベントを発火させず削除イベントのみを発火する", () => {
        const onSelect = vi.fn();
        const onDelete = vi.fn();
  ...
  describe("ActionFooter（アクションフッター）", () => {
    let container: HTMLDivElement | null = null;
    let root: ReactDOM.Root | null = null;
    ...
    describe("正常系テスト", () => {
      ...
      it("ローディング状態時に指定のローディングテキストが表示される", () => {
  ...
  describe("Accordion（アコーディオン）", () => {
    ...
    describe("正常系テスト", () => {
      it("初期状態が開いている場合（initiallyOpen=true）は各項目と上向きアイコンが描画される", () => {
        act(() => {
          root!.render(
            <Accordion
  ...
  describe("SearchTopView（検索トップ画面）", () => {
    let container: HTMLDivElement | null = null;
    let root: ReactDOM.Root | null = null;
    ...
    describe("正常系テスト", () => {
      it("デフォルトで詳細条件アコーディオンが開いており燃費や単価の項目が表示される", () => {
        act(() => {
          root!.render(
            <SearchTopView
              origin="東京駅"
    ...
    describe("境界値テスト", () => {
      it("初期状態を閉じる指定（conditionsAccordionInitiallyOpen=false）にした場合はアコーディオン内部の項目が表示されない", () => {
        act(() => {
          root!.render(
            <SearchTopView
              origin="東京駅"
